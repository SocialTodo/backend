...
class CachedFacebookBackend(object):
  ...
  def authenticate(self, request, token=None):
    try:
      ...
      if not user is None and user.facebook_token == token:
        logging.debug("User {} successfully logged in via CachedFacebookBackend!".format(user_provided_id))
        return user
      logging.debug("User {} does have an account, but the provided token did not match the cached token!".format(user_provided_id))
      return None
    except User.DoesNotExist:
      logging.debug("User {} does not yet have an account!".format(user_provided_id))
      return None
...
class FacebookBackend(object):
  ...
  def authenticate(self, request, token=None):
    try:
      ...
      user_provided_token, user_provided_id = CachedFacebookBackend.parse_request(self,request)
      if facebook_response == user_provided_id:
        user,created = User.objects.get_or_create(facebook_user_id = user_provided_id)
        if created:
        ...
        logging.debug("User {} failed to authenticate with Facebook! Could the token be expired?".format(user_provided_id))
        return None
    return None
  ...
  def query_facebook(self, token):
    ...
    request_successful = bool(fb_graph_dictionary["data"]["is_valid"])
    logging.debug("Facebook graph request returned {}".format(request_successful))
